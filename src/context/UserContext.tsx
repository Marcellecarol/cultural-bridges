...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [inventory, setInventory] = useState<Artifact[]>(() => getInitialState('inventory', [
    ...
    { id: '2', name: 'Ancient Rain Stick', missionId: 2, rarity: 'Epic', unlocked: false, shared: false },
  ...
  }, [hasOnboarded, userName, language, xp, streak, routesDone, inventory, missions, aiMode, highContrast]);

  const [communities, setCommunities] = useState<Community[]>([
    { id: 1, name: 'Kalunga Community', people: 'Kalunga People', country: 'BR', status: 'visited', desc: 'Largest quilombola territory in Brazil', x: 30, y: 40 },
    { id: 2, name: 'Xingu Park', people: 'Kamayurá People', country: 'BR', status: 'available', desc: 'Indigenous territory in Mato Grosso', x: 20, y: 35 },
    { id: 3, name: 'Rio Negro', people: 'Tukano People', country: 'BR', status: 'locked', desc: 'Northwestern Amazon region. (Unlocks at Lvl 4)', x: 15, y: 25 },
    { id: 4, name: 'Qiandongnan', people: 'Miao People', country: 'CN', status: 'visited', desc: 'Autonomous prefecture in Guizhou', x: 75, y: 30 },
  ]);
  ...
  const translatedInventory = inventory.map(item => {
    ...
    if (item.id === '2') return { ...item, name: language === 'ZH' ? '古老的祈雨棍' : 'Ancient Rain Stick' };
  ...
  return (
    <UserContext.Provider value={{
      ...
      isOffline, language, hasOnboarded, userName, 
      rewardToClaim: translatedRewardToClaim, 
      completeMission, exploreCommunity, completeRoute, toggleOffline, setLanguage, completeOnboarding, claimReward, shareArtifact, aiMode, toggleAiMode, highContrast, toggleHighContrast
    }}>
      {children}
